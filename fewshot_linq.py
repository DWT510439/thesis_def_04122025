...
fewshot_linq.py
----------------
Few-shot tuning scaffold for the Linq embedder using SentenceTransformers
and Optuna. The script performs the following tasks:
- Load a small fraction of the training data as few-shot examples
- Configure and run Optuna (Sobol init -> TPE) to optimize training
- Estimate FLOPs for budgeting and pareto analysis
- Train and evaluate models with SentenceTransformerTrainer
- Produce visualizations (Pareto front, training vs validation, fANOVA)

This file focuses on readability and experiment reproducibility; it
assumes local access to a ChromaDB collection for evaluation.
"""
...
COLLECTION_NAME = "baseline_embeddings_linq"
VALIDATION_PATH = "data/validation.jsonl"  

# MODEL
EMBED_MODEL = "Linq-AI-Research/Linq-Embed-Mistral"
OUTPUT_DIR = "/workspace/fewshot_model/"
RESULTS_DIR = "results/optuna_plots_30_fewshot_linq_2910/"
RANDOM_SEED = 28
random.seed(RANDOM_SEED) 

# STUDY NAME + RESULTS
STUDY_NAME = "30_fewshot_linq_2910_optuna"  # ← change per study run
OPTUNA_DB_PATH = os.path.join(RESULTS_DIR, f"{STUDY_NAME}.db")

# TUNING
FEW_SHOT_PERCENTAGE = 0.3  # Use subset for faster tuning
BEST_VAL_NDCG = 0.0
...
def log_trial_results(trial_number, params, metrics, filepath, max_vram_gb, total_flops, metrics_train, compute_time_sec):
    ...
    This records the hyperparameters, validation and train metrics, resource
    usage (VRAM) and FLOP estimates for later analysis.
...
def sample_train_eval(path, n=200):
    ...
    The function returns query texts and their matching chunk_id values (as
    strings) which are used as ground-truth ids for quick in-memory evaluation.

    Args:
        path (str): Path to the training JSONL file.
        n (int): Maximum number of examples to sample.

    Returns:
        tuple: (queries: List[str], gt_ids: List[str])
    """
    with open(path, "r", encoding="utf-8") as f:
        data = [json.loads(line) for line in f if line.strip()]
...
def examples_to_triplet_dataset(examples):
    ...
    This is useful because SentenceTransformerTrainer can accept a HF
    Dataset for training when triplet fields are explicitly named.
    """
    data = []
    for e in examples:
...
def compute_ndcg_in_memory(queries, model, doc_embeddings, db_ids, ground_truth_ids, k=TOP_K):
    """Compute nDCG@k and Recall@k for a set of queries using in-memory dot-products.

    Args:
        queries (List[str]): List of query strings.
        model (SentenceTransformer): A model used to encode queries.
        doc_embeddings (np.ndarray): Precomputed document embeddings (N x D).
        db_ids (List[str]): Corresponding document ids for the embeddings.
        ground_truth_ids (List[str]): Ground-truth chunk ids aligned with queries.
        k (Iterable[int]): Values of k for which to compute metrics (e.g., [1,5,10]).
...
def compute_ndcg_train_eval(queries, ground_truth_ids, model, doc_embeddings, db_ids, k=TOP_K):
    """Same as compute_ndcg_in_memory but intended for a small training sample.

    Kept separate to allow different logging, progress bars, or specialized
    handling of training-sample metrics.
    """
    ndcg_at_k = {kk: [] for kk in k}
    recall_at_k = {kk: [] for kk in k}
...
def objective(trial, validation_queries, ground_truth_ids, collection, train_eval_queries, train_eval_gt_ids):
    """Optuna objective function used to train a single trial.

    Steps performed:
    - Suggest hyperparameters
    - Sample few-shot training data and prepare a triplet dataset
    - Fine-tune a SentenceTransformer model with the suggested params
    - Re-embed the document collection and evaluate nDCG@k on the validation set
    - Optionally evaluate on the test set if a new best is found

    Returns:
        float: Validation nDCG@5 (used as the quantity to maximize).
    """
    ...
    if gflops is None:
        print("FLOP estimation failed — skipping FLOP scaling.")
        total_flops = 0  
    else:     
    ...
    trainer.train()
    # Re-embed documents with the fine-tuned model before evaluating
    ...
    print(f"Embedded all documents in the fine-tuned model")
...
# --------------------------------------------------------
    ...
    if ndcg > BEST_VAL_NDCG:
        ...
        if os.path.exists(TEST_PATH):
            ...
            print("No test.jsonl found — skipping test evaluation for best model.")
...
def main():
    """Entry point: prepare data, create an Optuna study, and run optimization.

    Main responsibilities:
    - Load validation and small train-eval sample
    - Connect to ChromaDB collection
    - Run Sobol initialization then TPE optimization
    - Save study artifacts and produce visualizations
    """
    ...
    try:
        # 1) Collect numeric parameters only
        numeric_params = set()
        ...
        if len(numeric_params) < 2:
            print("Not enough numeric parameters for fANOVA interactions.")
        else:
            print(f"Using numeric parameters: {', '.join(sorted(numeric_params))}")

            # 2) Create filtered trials (drop non-numeric params)
            filtered_trials = []
            for t in study.trials:
            ...
            # 3) Create temporary filtered study
        ...
        print(f"Could not compute fANOVA interactions: {e}")
