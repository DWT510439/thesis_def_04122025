...
RESULTS_DIR = "results/optuna_plots_10_fewshot_mini_40_trials_0411_test/"
RANDOM_SEED = 28
random.seed(RANDOM_SEED) #28 werkte erg goed

# STUDY NAME + RESULTS
STUDY_NAME = "30_fewshot_mini_0311_optuna"  # ← change per study run
...
SOBOL_TRIALS = 32
TPE_TRIALS =  8
FLOP_AVG_LGTH = 700 # average length in amount of tokens

# Hyperparameters for Bayesian Optimisation
LEARNING_RATE_LB = 8e-6
...
def compute_ndcg_train_eval(queries, ground_truth_ids, model, doc_embeddings, db_ids, k=TOP_K):
    ...
    for query, gt_id in tqdm(zip(queries, ground_truth_ids), total=len(queries), desc="Evaluating on train sample"):
        ...
        scores = (q_emb @ doc_embeddings.T).flatten()  # 1D vector
        ranked_idx = np.argsort(scores)[::-1]         # sorteer aflopend
        ranked_ids = [db_ids[i] for i in ranked_idx]
        ranked_scores = scores[ranked_idx]

        # relevance: 1 als het chunk_id overeenkomt, anders 0
        relevance = [1 if rid == str(gt_id) else 0 for rid in ranked_ids]
...
def objective(trial, validation_queries, ground_truth_ids, collection, train_eval_queries, train_eval_gt_ids):
    ...
    accumulation_steps = trial.suggest_categorical("accumulation_steps", ACCUMULATION_STEPS_OPTIONS)
    ...
    if gflops is None:
        ...
        total_flops = gflops * 3 * (len(train_dataset) // batch_size) * epochs
    
    trial.set_user_attr("total_flops", total_flops)

    print("DEBUG total_flops =", total_flops, type(total_flops))
    ...
    training_args = SentenceTransformerTrainingArguments(
            ...
            learning_rate=lr,
            lr_scheduler_type=scheduler,   # "linear", "cosine", "constant"
    ...
    trainer = SentenceTransformerTrainer(
        model=model,
        args=training_args,
        train_dataset=train_dataset,  # directly pass InputExample list
        loss=loss
    )
    ...
    # Extra check: performance on a training sample
    ...
    # # # Save best model
    # best_value = trial.study.best_value if len(trial.study.trials) > 0 and trial.study.trials[0].value is not None else 0
    # if ndcg > best_value:
